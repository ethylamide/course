...
data ParseResult a = ErrorResult ParseError | Result Input a
  deriving Eq
...
mapParser f p = P (\inp -> let parseResult = (parse p inp)
...
bindParser f p = P (\inp -> let parseResult = (parse p inp)
                           in case parseResult of
                           ErrorResult err -> ErrorResult err
                           Result inp' a -> case  (parse (f a) inp') of
                                            ErrorResult err' -> ErrorResult err'
                                            (Result inp'' b) -> (Result inp'' b))
...
(>>>) pa pb = P (\inp -> let parseResult = (parse pa inp)
                           in case parseResult of
                           ErrorResult err -> ErrorResult err
                           Result inp' _ -> (parse pb inp'))
...
(|||) p1 p2 = P (\inp -> let parseResult = (parse p1 inp)
                           in case parseResult of
...
list1 p = P (\inp -> let parseResult = (parse p inp)
                      in case parseResult of
                         ErrorResult err -> ErrorResult err
                         Result inp' v -> case inp' of
                                          Nil -> Result inp' (v :. Nil)
                                          _   -> case (parse (list p) inp') of
                                                 ErrorResult err' -> ErrorResult err'
                                                 Result inp'' v'' -> Result inp'' (v :. v''))
...
satisfy f = bindParser (\a -> if (f a)
                              then (valueParser a)
                              else (unexpectedCharParser a)) character
...
-- True
natural :: Parser Int
natural = bindParser (\a -> case (read a) of
                            Empty -> failed
...
sequenceParser ps = foldRight (\p r ->
                                 bindParser (\a ->
                                               bindParser (\r' ->
                                                             valueParser (a :. r'))
                                               r)
                                 p) (valueParser Nil) ps
...
firstNameParser = bindParser (\a -> bindParser (\b -> valueParser (a :. b)) (list lower)) upper
...
surnameParser = bindParser (\a ->
                              bindParser (\b ->
                                            bindParser (\c ->
                                                          valueParser (a :. b ++ c))
                                            (list lower))
                              (thisMany 5 lower)) upper
...
phoneParser = bindParser (\a -> bindParser (\b -> bindParser (\_ -> valueParser (a :. b)) (is '#')) phoneBodyParser) digit
-- phoneParser = do
--   a <- digit
--   b <- phoneBodyParser
--   _ <- is '#'
...
instance Apply Parser where
  ...
  (<*>) f a = P (\inp -> let parseResult = (parse f inp)
                         in case parseResult of
                            ErrorResult err -> ErrorResult err
                            Result inp' g -> case parse (g <$> a) inp' of
                                             ErrorResult err' -> ErrorResult err'
                                             other -> other)
