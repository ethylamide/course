...
instance Functor (State s) where
  ...
  (<$>) f state = State $ \s -> let (a, st) = runState state s
                                in (f a, st)
...
instance Apply (State s) where
  ...
  (<*>) f x = State $ \s -> let (g, fs) = runState f s
                                (a, xs) = runState x fs
                            in (g a, xs)
...
instance Bind (State s) where
  ...
  (=<<) f x = State $ \s -> let (a, xs) = runState x s
                            in runState (f a) xs
...
exec :: State s a -> s -> s
exec x s = let (_, xs) = runState x s
           in xs
...
firstRepeat xs = eval (findM withSetState xs) S.empty
  where
    withSetState :: Ord a => a -> State (S.Set a) Bool
    withSetState x = State $ (\s -> (x `S.member` s, S.insert x s))
