...
toSpecialCharacter c = let table = ('b', BackSpace) :.
                                   ('f', FormFeed) :.
                                   ('n', NewLine) :.
                                   ('r', CarriageReturn) :.
                                   ('t', Tab) :.
...
jsonString = between (charTok '"') (charTok '"') (list str)
  where
    str = do
      c1 <- character
      case c1 of
...
jsonNumber = P (\inp -> case readFloats inp of
                        Empty -> ErrorResult Failed
                        Full (n, inp') -> Result inp' n)
...
jsonValue = do
  spaces
  jsonValue'
  where jsonValue' = (JsonNull <$ jsonNull) |||
                     (JsonTrue <$ jsonTrue) |||
                     (JsonFalse <$ jsonFalse) |||
