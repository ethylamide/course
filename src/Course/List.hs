...
seqOptional xs = seqOptional' xs Nil
  where
    seqOptional' Nil zs = Full (reverse zs)
    seqOptional' (Empty :. _) _ = Empty
    seqOptional' ((Full val) :. ys) zs = seqOptional' ys (val :. zs)
...
lengthGT4 xs = lengthGT xs 4
  where
    lengthGT :: List a -> Int -> Bool
    lengthGT Nil _ = False
    lengthGT (_ :. ys) len | len > 0 = lengthGT ys (len - 1)
                           | otherwise = True
...
permutations xs0 = let perms Nil _ = Nil
                       perms (t:.ts) is =
