...
acceptL :: Lens (Env a) Accept
acceptL = Lens (\(Env _ s a) x -> Env x s a)
          (\(Env x _ _) -> x)

clientsL :: Lens (Env a) (IORef (Set Ref))
clientsL = Lens (\(Env x _ a) s -> Env x s a)
           (\(Env _ s _) -> s)

envvalL :: Lens (Env a) a
envvalL = Lens (\(Env x s _) a -> Env x s a)
          (\(Env _ _ a) -> a)

instance HandleLens (Env a) where
  handleL = acceptL .@ handleL

instance Functor Env where
  fmap f (Env x s a) = Env x s (f a)
