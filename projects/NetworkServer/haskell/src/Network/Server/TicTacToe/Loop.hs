...
instance Monad f => Monad (Loop v s f) where
  ...
  Loop k >>= f = Loop (\env s -> k env s >>= \(a, t) ->
                         let Loop l = f a
                         in l env t)
...
server :: IO w -- server initialise
       -> (w -> IO v) -- client accepted (pre)
       -> s -- initial state
       -> IOLoop v s () -- per-client
       -> IO a
server i r t l = let hand s w c = forever $
                     do q <- accept' s
...
perClient :: IOLoop v s x -- client accepted (post)
          -> (String -> IOLoop v s a) -- read line from client
          -> IOLoop v s ()
perClient q f = let lp = do k <- etry lGetLine
...
purgeClients a = mapM_ (\y -> ecatch (a y)
...
ecatch :: Exception e =>
       ...
       -> (e -> IOLoop v s a)
       -> IOLoop v s a
ecatch (Loop k) f = Loop $ \env s -> k env s `catch` (\e -> let Loop l = f e in l env s)

modifyClients :: (Set Ref -> Set Ref) -> IOLoop v s (Set Ref)
modifyClients f = initLoop $ \env ->
